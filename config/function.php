<?php
include "koneksi.php";
// $db = mysqli_connect("localhost", "root", "", "db_warung");
// global $mysqli;
...
function kd_cust_pemesanan()
{
	global $mysqli;
	// global $db;
	// $sql = $mysqli->query("SELECT MAX(RIGHT(kd_customer,3)) AS kdcust FROM customers");
	// $data = mysqli_fetch_assoc($sql);
	// $m = $sql->fetch();

	$query = mysqli_query($mysqli, "SELECT MAX(RIGHT(kd_customer,3)) AS kd_cust FROM customers");
	// $query = mysqli_query($mysqli, "SELECT * FROM customers");
	...
	if ($m['kd_cust'] <> NULL) {
		$kd = number_format($m['kd_cust'], 0) + 1;
		if (strlen($kd) == 1) {
			$no = "CUS" . "0000" . $kd;
		} elseif (strlen($kd) == 2) {
			$no = "CUS" . "000" . $kd;
